import { addChatSettingsListener, addDeleteChatListener, addRenameChatListener, addStarredChatsListener, addToProjectButtonListener, removeInitialContent, renderMessage } from "./chat.js";
import { chats, currentChatId, changeCurrentChatId } from "./init.js";
const recentsChatsContainer = document.querySelector('.recents-chats-container');
const recentsChatsLabel = document.querySelector('.recents-chats-label');
const starredChatsContainer = document.querySelector('.starred-chats-container');
const starredChatsLabel = document.querySelector('.starred-chats-label');
...
export function renderChats(action = '') {
    ...
    let recentsChatsHTML = ``;
    let starredChatsHTML = ``;
    chats.toReversed().forEach(chat => {
        ...
        <div class="chat-container ${chat.chatId === currentChatId ? 'current-chat' : ''}">
          ...
          <div class="dropdown-menu dropdown-menu-chat-id-${chat.chatId}">
            <button
              class="drowpdown-menu-item-button can-focus" id="star-chat-button"
    ...
    recentsChatsContainer.innerHTML = recentsChatsHTML;
    starredChatsContainer.innerHTML = starredChatsHTML;
...
export function addTypesChatsLabel() {
    if (chats.length === 0) {
        recentsChatsLabel?.classList.remove('active');
        starredChatsLabel?.classList.remove('active');
        return;
    }
    const { starredChatsQuantity, recentsChatsQuantity } = calculateSpecificTypeChatsQuantity();
    if (starredChatsQuantity !== 0 &&
        !starredChatsLabel?.classList.contains('active')) {
        starredChatsLabel?.classList.add('active');
    }
    else if (starredChatsQuantity === 0) {
        starredChatsLabel?.classList.remove('active');
    }
    if (recentsChatsQuantity !== 0 &&
        !recentsChatsLabel?.classList.contains('active')) {
        recentsChatsLabel?.classList.add('active');
    }
    else if (recentsChatsQuantity === 0) {
        recentsChatsLabel?.classList.remove('active');
    }
}
function calculateSpecificTypeChatsQuantity() {
    let starredChatsQuantity = 0;
    let recentsChatsQuantity = 0;
    chats.forEach(chat => {
        if (chat.starred) {
            starredChatsQuantity++;
        }
        else {
            recentsChatsQuantity++;
        }
    });
    return { starredChatsQuantity, recentsChatsQuantity };
}

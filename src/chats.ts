import { 
  addChatSettingsListener, addDeleteChatListener,
  addRenameChatListener, addStarredChatsListener, addToProjectButtonListener,
  removeInitialContent, renderMessage 
} from "./chat.js"
import { 
  chats, currentChatId,
  changeCurrentChatId
...
function calculateSpecificTypeChatsQuantity() {
  let starredChatsQuantity: number = 0
  let recentsChatsQuantity: number = 0
  chats.forEach(chat => {
    if (chat.starred) {
